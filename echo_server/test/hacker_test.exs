defmodule HackerTest do
  ...
  test "insert and retrieve requests" do
    ...
    :ok = :gen_udp.send(socket, {127, 0, 0, 1}, 5002, "foo=1")
    :ok = :gen_udp.send(socket, {127, 0, 0, 1}, 5002, "foo")
    assert {:ok, {_address, _port, "foo=1"}} = :gen_udp.recv(socket, 0)

    :ok = :gen_udp.send(socket, {127, 0, 0, 1}, 5002, "foo=2")
    :ok = :gen_udp.send(socket, {127, 0, 0, 1}, 5002, "foo")
  ...
  test "version" do
    ...
    :ok = :gen_udp.send(socket, {127, 0, 0, 1}, 5002, "version=foo")
    :ok = :gen_udp.send(socket, {127, 0, 0, 1}, 5002, "version")
